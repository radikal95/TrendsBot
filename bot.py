...
def default_test(message):
    ...
    #keyboard = types.InlineKeyboardMarkup(row_width=1)1
    ...
    #bot.send_message(message.chat.id, "Добро пожаловать в бот стратегического семинара МегаФон. Для обратной связи следуйте подсказкам бота.",reply_markup=keyboard)
...
def callback_inline(call):
    ...
    if call.message:
        if call.data == "0":
            ...
            url_button4 = types.InlineKeyboardButton(text="5. Прочее", callback_data=5)
            keyboard.add(url_button, url_button1, url_button2, url_button3, url_button4)
            bot.send_message(call.message.chat.id, "Для обратной связи следуйте подсказкам бота.", reply_markup=keyboard)
        if call.data == "1":
...
def default_test(message):
    ...
    if config.waiting[config.users.index(message.chat.id)] and config.name[config.users.index(message.chat.id)]!='' :
